 ...
 * Provides methods for calculating average grades, finding best and worst
 * grades, finding specific courses and providing all courses associated with
 * student as an ObersableList.
 * 
...
public class Student {
     ...
     * 
     * @param name A string that sets the internal name/nickname of the Student
     *             object.
     ...
     * 
     * @param n A interger corresponding to the Course object index to retrieve
     *          from the Student arrayList
     ...
     * 
     * @param n A int corresponding to Course Object's index that
     *          we wish to remove.
     */
    ...
    /**
     * Remove a course from the list of assigned courses to this
     * Student Object.
     * 
     * @param removeCourse A course object that is to be removed
     *                     from the Student.
     */
    ...
    public double getTotalUsableCoursePoints() {
        double totalPoints = 0;
        for (Course thisCourse : this.myGrades) {
            if (!thisCourse.getCourseGrade().equals("Best�tt")) {
                totalPoints = totalPoints + thisCourse.getCoursePoints();
            }
        }
        return totalPoints;
     ...
     * @param grade       is a String of A-F or "Best�tt".
     * @throws IllegalArgumentException in case of invalid inputs.
     */
    public void addNewGrade(
            String subjectName, String subjectCode, String grade) {
     ...
     * @param subjectCode A courseCode that is validated before acceptance.
     * @param grade       is a String of A-F or "Best�tt".
     * @param points      A double of the points a course would give the
     *                    student.
     * @throws IllegalArgumentException in case of invalid inputs.
     */
    public void addNewGrade(String subjectName, String subjectCode,
    ...
    /**
     * Return the worst grade obtained (only A-F, pass grades does not count)
     * for the Student.
     * 
     * @return String contained the worst grade (A-F).
     */
    public String getWorstGrade() {
        // fixed value set since this is best allowed grade.
        char worstGrade = 'A';
        for (Course thisCourse : myGrades) {
            if (!thisCourse.getCourseGrade().equals("Best�tt")) {
    ...
    /**
     * Return the best grade obtained (only A-F, pass grades does not count)
     * for the Student.
     * 
     * @return String contained the best grade (A-F).
     */
    public String getBestGrade() {
        // fixed value set since this is worst allowed grade.
        char bestGrade = 'F';
        for (Course thisCourse : myGrades) {
            if (!thisCourse.getCourseGrade().equals("Best�tt")) {
    ...
    /**
     * Calculate the average grade for the Student based on all courses
     * assigned the student with a A-F grade, Pass grades are not included in
     * calculation.
    ...
    public String getAverageGrade() {
        // Initialization of point summation used in loop.
        double totalPoints = 0;
        // Look at all courses in this.myGrades
        for (Course thisCourse : this.myGrades) {
            if (!thisCourse.getCourseGrade().equals("Best�tt")) {
                double weightedPoints = fromGradeToInt(
                        thisCourse.getCourseGrade())
                        * thisCourse.getCoursePoints();
                totalPoints = totalPoints + weightedPoints;
            }
        }
        double averageGradePoints = Math.round(
                totalPoints / this.getTotalUsableCoursePoints());
        // Convert from Double to String
        String averageCharGrade = this.fromDoubleToGrade(averageGradePoints);
        return averageCharGrade;
    }

    /**
     * Internal function that convert from a provided String grade
     * (A-F or Bestått) to a integer that can be directly used in calculations.
     * 
    ...
    /**
     * Internal function that convert from a provided double to a String grade
     * (A-F or Bestått).
     * 
     * @param n The double that we wish to convert to a String grade.
     * @return A string with the corresponding grade (A-F or Best�tt)
     */
    ...
    /**
     * Find and return a Course object from the assigned Courses to the Student
     * object based on the Course code.
     * 
    ...
    public Course findCourseUsingCourseCode(String findThisCourseCode)
        ...
        for (int i = 0; i < this.getCourseAmount(); i++) {
            if (this.getCourse(i).getCourseCode().equals(
                    (findThisCourseCode).trim())) {
        ...
        if (targetCourse != null) {
            ...
            throw new IllegalArgumentException(
                    "Could not find the Course with the course code \""
                            + findThisCourseCode
                            + "\"");
        }
    }
    ...
    public ObservableList<Course> getObservableListOfCourses() {
        ObservableList<Course> listedCourses = FXCollections
                .observableArrayList();
        for (int i = 0; i < this.getCourseAmount(); i++) {
    ...
    /**
     * Return a formatted string that contain the average grade,
     * the worst grade, and the best grade for the input Student object.
     * 
     * @param studentGrades The student and their associated Course objects
     *                      that we calculate average grade for.
    ...
    public String outputFormattedGradesString(Student studentGrades) {
        ...
        if (studentGrades.getCourseAmount() == -1) {
            ...
            outputMessage = "Din gjennomsnittskarakter er: "
                    + studentGrades.getAverageGrade()
                    + "\nBeste karakter: "
                    + studentGrades.getBestGrade()
                    + "\nVærste karakter: "
                    + studentGrades.getWorstGrade();
        }
